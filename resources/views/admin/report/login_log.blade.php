@extends("admin.base")
@section("content")
<!-- Upcoming Appointments -->
<div class="col-12">
    <div class="border-top mb-4 pt-3">
        <h5 class="fw-medium text-uppercase mb-0">Login Log Report</h5>
        <p class="fs-12">Login Log Report</p>
        <div class="row g-lg-12 g-3">
            <div class="col-lg-12">
                {{ html()->form('POST')->route('report.login.log.fetch')->class('')->open() }}
                <div class="row g-3">
                    <div class="control-group col-md-2">
                        <label class="form-label req">From Date </label>
                        {{ html()->date('from_date', old('from_date') ?? $inputs[0])->class('form-control') }}
                        @error('from_date')
                        <small class="text-danger">{{ $errors->first('from_date') }}</small>
                        @enderror
                    </div>
                    <div class="control-group col-md-2">
                        <label class="form-label req">To Date </label>
                        {{ html()->date('to_date', old('to_date') ?? $inputs[1])->class('form-control') }}
                        @error('to_date')
                        <small class="text-danger">{{ $errors->first('to_date') }}</small>
                        @enderror
                    </div>
                    <div class="control-group col-md-2">
                        <label class="form-label">User </label>
                        {{ html()->select('user_id', $users, old('user_id') ?? $inputs[2])->class('select2') }}
                    </div>
                </div>
                <div class="raw mt-3">
                    <div class="col text-end">
                        {{ html()->button('Cancel')->class('btn btn-secondary')->attribute('onclick', 'window.history.back()')->attribute('type', 'button') }}
                        {{ html()->submit('Fetch')->class('btn btn-submit btn-primary') }}
                    </div>
                </div>
                {{ html()->form()->close() }}
            </div>
        </div>
    </div>
</div>
<div class="col-12">
    <div class="border-top mb-4 pt-3">
        <h5 class="fw-medium text-uppercase mb-0">Login Log Report</h5>
        <p class="fs-12">Showing Login Log between {{ $inputs[0] }} and {{ $inputs[1] }}</p>
        <div class="row g-lg-4 g-3">
            <div class="col-lg-12">
                <table class="Data_Table table table-round align-middle mb-0 table-hover w-100 mt-2 border-top" id="apptTable">
                    <thead>
                        <tr>
                            <th class="py-2 fw-medium small text-uppercase">SL No</th>
                            <th class="py-2 fw-medium small text-uppercase">Date</th>
                            <th class="py-2 fw-medium small text-uppercase">Branch</th>
                            <th class="py-2 fw-medium small text-uppercase">Device</th>
                            <th class="py-2 fw-medium small text-uppercase">User</th>
                            <th class="py-2 fw-medium small text-uppercase">IP</th>
                            <th class="py-2 fw-medium small text-uppercase">Place</th>
                            <th class="py-2 fw-medium small text-uppercase">Logged In</th>
                            <th class="py-2 fw-medium small text-uppercase">Logged Out</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($records as $key => $item)
                        <tr>
                            <td>{{ $key + 1 }}</td>
                            <td>{{ $item->created_at->format('d.M.Y') }}</td>
                            <td>{{ $item->branch?->name }}</td>
                            <td>{{ $item->user_agent }}</td>
                            <td>{{ $item->user->name }}</td>
                            <td>{{ $item->ip_address }}</td>
                            <td>{{ $item->city .' | '. $item->region .' | '. $item->country }}</td>
                            <td>{{ $item->logged_in_at?->format('d.M.Y h:i a') }}</td>
                            <td>{{ $item->logged_out_at?->format('d.M.Y h:i a') }}</td>
                        </tr>
                        @empty
                        @endforelse
                    </tbody>
                </table>
            </div>
        </div>
    </div>
</div>
@endsection