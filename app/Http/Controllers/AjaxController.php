...
class AjaxController extends Controller
{
    ...
    function getRegOrder(Request $request)
    {
        ...
        })->where('e.id', '<=', 12)->selectRaw("LAST_DAY(CURRENT_DATE) + INTERVAL 1 DAY - INTERVAL e.id MONTH AS date, COUNT(CASE WHEN o.deleted_at IS NULL THEN o.id END) AS ocount, CONCAT_WS('/', DATE_FORMAT(LAST_DAY(CURRENT_DATE) + INTERVAL 1 DAY - INTERVAL e.id MONTH, '%b'), DATE_FORMAT(LAST_DAY(CURRENT_DATE) + INTERVAL 1 DAY - INTERVAL e.id MONTH, '%y')) AS month")->groupBy('date', 'e.id')->orderByDesc('date')->get();
